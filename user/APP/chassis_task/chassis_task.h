...
typedef struct
{
  const RC_ctrl_t *chassis_RC;               //底盘使用的遥控器指针   		pointer to the RC used
  ...
  fp32 vx;                         //底盘速度 前进方向 前为正，单位 m/s  			chassis speed: forward as positive, unit m/s
  fp32 vy;                         //底盘速度 左右方向 左为正  单位 m/s  			chassis speed: left as positive, unit m/s
  fp32 wz;                         //底盘旋转角速度，逆时针为正 单位 rad/s  	chassis rotational speed: counter clockwise as positive, unit rad/s
  fp32 vx_set;                     //底盘设定速度 前进方向 前为正，单位 m/s  		set forward/back speed
